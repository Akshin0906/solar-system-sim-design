...
// Deterministic contact and debris policy. Keeping this module independent of the
// gravity step prevents impact heuristics from obscuring the integrator's hot loop.
...
export const recordEvent = (state: IntegratorState, event: IntegratorState["events"][number]) => {
...
export const handleCollisions = (state: IntegratorState) => {
...
// Cool molten remnants and impact afterglows in simulation time. This stays beside the
// contact policy that creates moltenHeat while the fixed-step loop controls call order.
export const coolMoltenBodies = (state: IntegratorState, dt: number) => {
